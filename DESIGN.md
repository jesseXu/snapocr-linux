...
5. **收尾** —— 写 COSMIC 快捷键配置、autostart、托盘图标、设置界面。
...
- 主程序用 Rust 还是 Python —— 等第 1 步做完，看 `snapocr-shot` 的实现体量再定。
- 保存目录：跟随 XDG `XDG_PICTURES_DIR`，还是像 macOS 版那样固定桌面？
- v1 是否需要托盘图标与设置界面，还是纯快捷键驱动 + 配置文件即可。
