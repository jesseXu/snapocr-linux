...
toast 的内容进一步做成**纯图标 + 数字**（剪贴板对勾、`[S]` 键帽 + 下载图标、
`[E]` 键帽 + 笔图标）。这不只是观感取舍：图标不需要翻译，也就不需要字体发现、
排版与 CJK 回退那一整套 —— 原先为画几行中文引入的 `cosmic-text` 因此可以整个
去掉，二进制从 5.2MB 回到 1.9MB，还省掉启动时扫描系统字体的开销。
...
**一律英文，不做多语言。** 一个人用的小工具，i18n 的收益远小于维护成本。
自绘的 toast 更进一步做成纯图标 + 数字，连英文都省了。标注编辑器的三个
工具按钮同理用图标 —— 而且图标是用编辑器自身的绘制函数画的，图标就是
该工具的真实笔迹，既不依赖图标主题（缺失会变成空白按钮），也不会画错。

代码注释与本文档保持中文：那是写给维护者的，不是界面。
